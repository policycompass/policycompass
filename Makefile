...
PYTHON_EXECUTABLE=$(which python3.4)
...
UBUNTU_PACKAGES= maven tomcat7 libxml2 libxslt1.1 libzip2 python3 python3-pil python3-pip python-virtualenv \
	         python3-ipdb python3-pep8 pyflakes sqlite build-essential zlibc curl file git \
	         ruby nodejs npm leiningen openjdk-7-jdk phantomjs supervisor nginx
...
install_deps_ubuntu:
	apt-get -y update
	apt-get -y install $(UBUNTU_PACKAGES)x
...
adhocracy_install: adhocracy3/bin/buildout
...
carneades_install: nix_build
	...
	ln -s $(CATALINA_EXECUTABLE) ./bin/catalina.sh
...
.PHONY: test_install frontend_install adhocracy_git adhocracy_install postgres_init fcmmanager_install all install_deps install_elasticsearch_ubuntu install_deps_ubuntu
