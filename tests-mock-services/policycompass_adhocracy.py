"""Adhocracy services dummy rest server."""
from wsgiref.simple_server import make_server
from pyramid.config import Configurator
from pyramid.view import view_config


@view_config(request_method='POST', route_name='login_email',
             renderer='json', accept="application/json")
@view_config(request_method='POST', route_name='login_username',
             renderer='json', accept="application/json")
...
@view_config(request_method='POST', route_name='principals_users',
             renderer='json', accept="application/json")
def post_principals_users(request):
    ...
    passwd = data['adhocracy.sheets.user.IPasswordAuthentication']['password']
    assert len(passwd) >= 6
